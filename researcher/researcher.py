...
class Researcher(GPT): 
  ...
  def editTextMultistep(self, text, objectives = []):
    ...
    objectives = editor_objectives if len(objectives) == 0 else objectives

    for objective in objectives:
      text = self.editText(text, objective = objective)
  ...
  def getDataframeFacts(self, df, metric_name, days = [7, 30, 90, 180], rounding=3, prefix='', metric_renames={}):
    facts = []
    
    for column in df.columns:
      for day_count in days: 
          t0 = round(df[column].iloc[-1].sum(), rounding)
          tn = round(df[column].iloc[-day_count].sum(), rounding)

          percent_chg = df[column].pct_change(day_count, fill_method='ffill').iloc[-1]
          percent_chg = round(percent_chg * 100, 1)

          metric_ = metric_renames.get(metric_name, metric_name)

          fact = f'{prefix} {column} has {t0:,.0f} {metric_} with a {day_count}d percent change of {percent_chg}% ### '
          facts.append(fact)

    return facts
  

  def getAllDataframeFacts(self, df, group_column, metrics_column, value_column, date_column = 'date', days = [7, 30, 90, 180], moving_average_days=1, rounding=3, metric_renames={}, prefix = '', group_col_count = 0):

    # -- initialize facts list
    facts = []

    # -- get unique metrics 
    metrics = df[metrics_column].unique() 

    # -- loop through metrics and dates 
    for metric in metrics:
        # -- filter df for metric 
        temp_df = df[df[metrics_column] == metric]

        # -- reshape df and group by date and column
        temp_df = temp_df.groupby([date_column, group_column])[value_column].sum().unstack().fillna(0)

        # -- calculate moving average
        temp_df = temp_df.rolling(moving_average_days).mean()

        # -- limit columns if group_col_count is defined
        if group_col_count > 0:
          temp_df = temp_df[temp_df.columns[temp_df.fillna(0).astype(int).iloc[-1].argsort()]]
          temp_df = temp_df[temp_df.columns[::-1]]

          # -- limit columns to top 10
          temp_df = temp_df[temp_df.columns[:group_col_count]]

        # -- get facts for each day count defined 
        facts = facts + self.getDataframeFacts(temp_df, metric, days, rounding, prefix, metric_renames)

    return facts 
  
  def describeDataframeFacts(self, facts, prompt = '', user_insight = '', temp = 0.3, model = 'chat', search_internet = False, use_knowledge_base = False): 
    facts_str = ' '.join(facts)

    # -- define default prompt
    base_prompt_ = """
      Generate a well written section of analysis based on only the important data facts provided. 
      Make sure to use compact notation for numbers for readability and remember 9 digits is a billion, 6-8 digits is a million and so on.
      Also please do not add metrics which should not be added together.
    """

    system_prompt = 'SYSTEM: you are a havard educated expert investment analysis who writes crisp, insightful analysis of data to invesment managers.'

    prompt_vars = ['DATA FACTS: {facts}']

    inputs = {'facts': facts_str}

    # -- load user prompt if define 
    base_prompt = base_prompt_ if prompt == '' else prompt
    
    if user_insight != '': 
      base_prompt = base_prompt + '\n' + 'Include in the generated analysis the insight provided by an expert user as they are very respected by the investment managers.'
      prompt_vars.append('USER INSIGHT: {user_insight}')
      inputs['user_insight'] = user_insight

    # -- build prompt
    prompt = system_prompt + ' \n\n ' + base_prompt + ' ### \n\n ' + ' ### \n '.join(prompt_vars) + ' ### \n\n ' + 'ANALYSIS:'

    # -- generate analysis
    analysis = self.generateTextLLM(prompt, inputs = inputs,  model = model, temp = temp)

    if use_knowledge_base == False:
      return analysis
    
    # -- TODO -- add internet search results to context

    # -- apply knowledge base to generated analysis 
    prompt_function = loadDataDescribePrompt
    inputs = {'analysis': analysis, 'context': ''}

    base_prompt = """
      Provided is an analysis conducted by your data analsyt colleague. Since you have a lot of qualitative CONTEXT knowledge about the data, please add your own analysis to the report. 

      Make sure to use compact notation for numbers for readability and remember 9 digits is a billion, 6-8 digits is a million and so on.
      Also please do not add metrics which should not be added together.
    """

    prompt_vars = ['DATA ANALYSIS: {analysis}', 'CONTEXT: {context}']
    inputs = {'analysis': analysis, 'context': ''}
