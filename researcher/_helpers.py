...
def standardizeLinkListFormat(link_list): 
  ...
  # -- list of dicts 
  if type(link_list[0]) == dict:
    source_objects = []

    for link_set in link_list:
      if 'link' in link_set.keys():
        link_set['links'] = [link_set['link']]
        del link_set['link']

      if 'title' not in link_set.keys():
        link_set['title'] = ''

      source_objects.append(link_set)
...
def combineSourcesText_(link_list, twitter_client, sleep =0.125): 
